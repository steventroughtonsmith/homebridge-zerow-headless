...
Camera.prototype.handleSnapshotRequest = function (request, callback) {
  ...
  // Need to set the exposure time before a snapshot or else you'll get a blown-out image

  this._v4l2CTLSetCTRL('auto_exposure', 1)
  this._v4l2CTLSetCTRL('exposure_time_absolute', 16)

  var date = new Date();
  var current_hour = date.getHours();

  /* Kinda hack to set scene for night time */
  if (current_hour > 20 || current_hour < 9)
  {
    this._v4l2CTLSetCTRL('scene_mode', 8)
  }
  else
  {
    this._v4l2CTLSetCTRL('scene_mode', 0)
  }
...
Camera.prototype.handleStreamRequest = function (request) {
  ...
  if (requestType === 'start' && this.pendingSessions[sessionIdentifier]) {
    ...
    this._v4l2CTLSetCTRL('auto_exposure', 0)
    this._v4l2CTLSetCTRL('scene_mode', 0)
